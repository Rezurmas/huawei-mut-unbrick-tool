...
<#
 Huawei WS7200 / AX3 - Unbrick Wizard ALL-IN-ONE (HuaweiMUT.exe LUB mut_python.py)

 Wersja po polaczeniu z mut_python.py - reverse-engineered klonem MUT'a.
 Origialny _unbrick_wizard.ps1 zostawiony nietkniety (do porownan).

 Backup, wybor firmware, walidacja CRC, regeneracja .ini (z opcja FORCE-CFE),
 telnet, firewall, wylaczenie WiFi/Tailscale, statyczny IP,
 URUCHOMIENIE: HuaweiMUT.exe (oryginalny) LUB mut_python.py (klon w Pythonie),
 monitoring, telnet diag restore, restore DHCP - wszystko interaktywnie.
#>
...
$mutPython     = Join-Path $scriptDir 'mut_python.py'
$iniGen        = Join-Path $scriptDir 'generate_ini_file.py'
$iniGenForce   = Join-Path $scriptDir 'generate_ini_force_cfe.py'
...
Clear-Host
Write-Section 'Huawei WS7200 - Unbrick Wizard ALL-IN-ONE'
Write-Log "Log: $logFile" 'DarkGray'
Write-Log "Folder: $scriptDir" 'DarkGray'

# Wybor wariantu MUT (na samym poczatku)
$useMutExe = $true
$usePyMut  = $false
if (Test-Path $mutPython) {
    Write-Host ""
    Write-Host "Wybierz wariant MUT:" -ForegroundColor White
    Write-Host "  [1] HuaweiMUT.exe         (oryginalny, default) <-- ZALECANE" -ForegroundColor Gray
    Write-Host "  [2] mut_python.py         (klon w Pythonie, eksperymentalne)" -ForegroundColor Gray
    Write-Host "  [3] OBA - najpierw .exe, jak nie zadziala to .py" -ForegroundColor Gray
    Write-Host "  [4] mut_python.py + verbose (debug, log kazdego packeta)" -ForegroundColor Gray
    $mutChoice = (Read-Host "Wybor [Enter = 1]").Trim()
    switch ($mutChoice) {
        '2' { $useMutExe = $false; $usePyMut = $true;  Write-Log "MUT: mut_python.py" 'Cyan' }
        '3' { $useMutExe = $true;  $usePyMut = $true;  Write-Log "MUT: oba (najpierw .exe)" 'Cyan' }
        '4' { $useMutExe = $false; $usePyMut = $true;  $pyVerbose = $true; Write-Log "MUT: mut_python.py (verbose)" 'Cyan' }
        default { Write-Log "MUT: HuaweiMUT.exe (default)" 'Cyan' }
    }
}

if (-not (Test-Path $iniGen)) { Write-Log "BLAD: brak $iniGen" 'Red'; Wait-Enter; exit 1 }
if ($useMutExe -and -not (Test-Path $mutPath)) { Write-Log "UWAGA: brak HuaweiMUT.exe pod $mutPath" 'Yellow' }
if ($usePyMut -and -not (Test-Path $mutPython)) { Write-Log "BLAD: brak mut_python.py pod $mutPython" 'Red'; exit 1 }
...
@(
 ...
 '2. Wybor firmware + walidacja .bin/.ini (CRC) + opcja FORCE-CFE',
 ...
 '7. Uruchomienie MUT (HuaweiMUT.exe lub mut_python.py)',
...
$bins = Get-ChildItem -Path $scriptDir -Filter 'WS7200-20_*.bin' -Recurse | Sort-Object Name
...
# Walidacja .ini + opcja FORCE-CFE
$iniFile = [System.IO.Path]::ChangeExtension($binFile.FullName, '.ini')
Write-Host ""
Write-Host "TRYB INI - WYBIERZ:" -ForegroundColor Yellow
Write-Host "  [1] Normalny  (CFE uncover) - zachowuje bootloader" -ForegroundColor Gray
Write-Host "  [2] FORCE-CFE (CFE cover)   - nadpisuje bootloader (dla cegly z popsutym CFE)" -ForegroundColor Gray
Write-Host "  [3] Zostaw istniejacy .ini bez zmian" -ForegroundColor Gray
$iniMode = (Read-Host "Tryb [Enter = 1]").Trim()
$forceCfe = ($iniMode -eq '2')
$skipRegen = ($iniMode -eq '3')

$regen = $false
if (-not (Test-Path $iniFile)) { Write-Log "Brak .ini - wymuszam regeneracje" 'Yellow'; $regen = $true }
elseif (-not $skipRegen) {
    if ($forceCfe) {
        Write-Log "Tryb FORCE-CFE - regeneruje .ini" 'Yellow'; $regen = $true
    } else {
...
if ($regen) {
    ...
    try {
        if ($forceCfe) {
            if (Test-Path $iniGenForce) {
                & python $iniGenForce $binFile.FullName | ForEach-Object { Write-Log "  $_" 'Green' }
            } else {
                # Fallback: uzyj mut_python.py --gen-ini --force-cfe
                & python $mutPython $binFile.FullName --gen-ini --force-cfe | ForEach-Object { Write-Log "  $_" 'Green' }
            }
        } else {
            & python $iniGen $binFile.Name | ForEach-Object { Write-Log "  $_" 'Green' }
        }
...
if ($forceCfe) {
    Write-Log "TRYB FORCE-CFE aktywny - INI zawiera CFE=cover" 'Yellow'
}
...
if ($useMutExe -and (Test-Path $mutPath)) {
...
if ($usePyMut -and $python) {
    foreach ($d in 'Inbound','Outbound') {
        $n = "PythonMUT-App-$d"
        if (-not (Get-NetFirewallRule -DisplayName $n -ErrorAction SilentlyContinue)) {
            New-NetFirewallRule -DisplayName $n -Direction $d -Program $python.Source -Action Allow -Profile Any | Out-Null
            Write-Log "  Dodano per-app (python): $n" 'Green'
        }
    }
}
...
# ===== KROK 7: URUCHOM MUT =====
Write-Section '[7/9] Uruchamianie MUT (flash firmware)'

# 7A: HuaweiMUT.exe (oryginalny)
if ($useMutExe -and (Test-Path $mutPath)) {
    Write-Host @"
###########################################################################
##                                                                       ##
...
##   (Chyba ze uzyles trybu FORCE-CFE - wtedy oba checkboxy juz ON)     ##
##                                                                       ##
###########################################################################
...
# 7B: mut_python.py (klon)
if ($usePyMut -and (Test-Path $mutPython)) {
    Write-Host ""
    Write-Host "===== mut_python.py (klon w Pythonie) =====" -ForegroundColor Cyan
    Write-Host @"
Skrypt mut_python.py jest reverse-engineered klonem HuaweiMUT.exe.
Wysyla pakiety UDP multicast (224.0.0.119:13456) z plikiem .bin.

Roznice vs original:
  - Loguje kazdy packet (z --verbose)
  - Nie ma GUI - dziala w konsoli
  - Bez 'Cover partitions' checkboxow - bierze z .ini PARTITIONS!
    (DLATEGO musisz uzyc trybu FORCE-CFE w .ini dla recovery cegly!)
  - Mozesz nacisnac Ctrl+C zeby zatrzymac w dowolnej chwili

"@ -ForegroundColor White

    if (-not $forceCfe) {
        Write-Host "OSTRZEZENIE: Nie wybrales FORCE-CFE w kroku 2." -ForegroundColor Yellow
        Write-Host "             mut_python.py wysle CFE=uncover (bootloader NIE bedzie nadpisany)." -ForegroundColor Yellow
        if (Read-YN 'Chcesz teraz zregenerowac .ini z FORCE-CFE?' $false) {
            & python $mutPython $binFile.FullName --gen-ini --force-cfe
        }
    }

    $pyArgs = @($mutPython, $binFile.FullName, '--pc-ip', $staticIP, '-v')
    if ($pyVerbose) { $pyArgs += '--hex' }
    Write-Host ""
    Write-Host "Komenda: python $($pyArgs -join ' ')" -ForegroundColor DarkGray
    Write-Host "(Ctrl+C zatrzymuje, kazda pelna runda = caly plik wyslany)" -ForegroundColor DarkGray
    Write-Host ""
    if (Read-YN 'Uruchomic teraz mut_python.py?') {
        Write-Log "  Uruchamiam mut_python.py..." 'Green'
        & python @pyArgs
        Write-Log "  mut_python.py zakonczony" 'Green'
    }
    Wait-Enter '>>> Po pomyslnym flashu (zielona dioda), nacisnij Enter <<<'
}
